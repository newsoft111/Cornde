    ...
    * Owl Corousel
    ...
    MOMB.CountTimer = function() {
			$('.count-down').each(function() {
				var $count_timer = $( this );
				var regionalVar = { days: 'Days', day: 'Day', years: 'Years', year: 'Year', hours: 'Hours', hour: 'Hour', minutes: 'Minutes', minute: 'Minute', seconds: 'Seconds', second: 'Second' };
				if ($count_timer.length > 0) {
					$count_timer.countdown({ regional: regionalVar });
				}
			});
    }
                ...
                if (input.files && input.files[0]) {
									...
									if (isSuccess) {			
										...
										reader.onload = function(e) {
											...
											$(".h-user-icon").children("img").attr("src", e.target.result);
											
											var queryString = {
												csrfmiddlewaretoken: Cookies.get('csrftoken'),
												avater: e.target.result,
											};
									
											$.ajax({
													type : 'POST',
													url : ChangeUserAvater,
													data : queryString,
													dataType : 'json',
...
function openPopup(subject, content, before_modal, option) {
	$("#modal_subject").text(subject);
	$("#modal_content").html(content);
	if (!isEmpty(before_modal)) {
		$('#closePopupBtn').attr('data-before-modal',before_modal);
	}
	if (option == 'reload') {
		document.querySelectorAll('#closePopupBtn').forEach(function(close_btn) {
...
function closePopup() {
	var before_modal = $('#closePopupBtn').attr('data-before-modal');
	$("#modal_subject").text('');
  $("#modal_content").html('');
	if (before_modal) {
		$(before_modal).modal('show');
	}
	$('#closePopupBtn').attr('data-before-modal','');
}
